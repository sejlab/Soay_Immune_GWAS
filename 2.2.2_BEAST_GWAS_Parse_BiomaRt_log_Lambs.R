...
nrow(snp.wald)/3 == nsnps(genabeldata)
...
for(i in unique(snp.wald$Model)){
  ...
  png(paste0("figs/2_GWAS_", i, "_log_Lambs.png"), width = 9, height = 4, units = "in", res = 300)
...
write.table(snp.wald.corrected, "results/2_GWAS_Results_log_Lambs.txt", row.names = F, sep = "\t", quote = F)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# 4. Format the gene tables                         #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

sig.snps <- subset(snp.wald.corrected, Pr.Chisq. < 2.245e-6 & Model == "IgEmp_Lambs")
...
write.table(sig.snps, "results/2_Regions_for_Imputation_log_lambs.txt", row.names = F, sep = "\t", quote = F)

save(full.fixef, full.n, full.ranef, full.wald, snp.wald.corrected, sig.snps, file  = "results/2_GWAS_Full_Results_log_Lambs.RData")
