...
models <- subset(models, LambAdult == "Lambs")
...
for(h in startsnp:(startsnp+99)){
  ...
  genotab <- data.frame(as.character.gwaa.data(genabeldata[,h]))
  head(genotab)
  names(genotab) <- "SNP"
  genotab$ID <- row.names(genotab)
  ...
  ped.results.ranef$SNP.Name <- snpnames(genabeldata)[h]
  ped.results.fixef$SNP.Name <- snpnames(genabeldata)[h]
  ped.results.wald$SNP.Name <- snpnames(genabeldata)[h]
  ped.results.n$SNP.Name <- snpnames(genabeldata)[h]
  ...
  if(h == nsnps(genabeldata)) save(restab.ranef, restab.fixef, restab.wald, restab.n, file = paste0("GWAS_", startsnp, "_log_Lambs.RData"))
  
}

save(restab.ranef, restab.fixef, restab.wald, restab.n, file = paste0("GWAS_", startsnp, "_log_Lambs.RData"))
