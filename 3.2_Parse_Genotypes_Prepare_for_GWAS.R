...
sig.regions <- read.table("results/2_Regions_for_Imputation.txt", header = T, stringsAsFactors = F)

sig.regions$Chromosome[which(sig.regions$Chromosome == 2 & sig.regions$Position > 2e8)] <- "2a"
...
save(full.imputedgenos, full.mapfile, file = "alphaimpute/Imputed_GWAS.RData")
