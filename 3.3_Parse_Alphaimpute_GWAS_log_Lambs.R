...
for(i in 1:nrow(freqinfo)){
  ...
  x <- data.frame(table(full.imputedgenos.log.lambs[,which(names(full.imputedgenos.log.lambs) == freqinfo$SNP.Name[i])]))
...
write.table(snp.wald, "../results/3_Imputed_GWAS_Results_log_Lambs.txt", row.names = F, sep = "\t", quote = F)
...
save(full.fixef.hd, full.n.hd, full.ranef.hd, full.wald.hd, file = "../results/3_HD_GWAS_Full_Results_log_Lambs.RData")
